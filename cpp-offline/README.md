...
- `kukot` keeps one documented local aesthetic override: the repaired Java source does not explicitly flip normals or culling, yet side-by-side review showed substantially closer body relief when the C++ port flips env-map normals and reverses torsion direction; backface culling still stays on the Java-side contract
