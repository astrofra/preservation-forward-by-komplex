...
- `kukot` now loads `asses/under1.ase`, `images/envplane.gif`, and `images/flare1.jpg` directly in C++, then renders a substantially closer source-shaped pass with looped spline-like track sampling, accumulated object rotations, Java-style `jAkKAma = 2` mesh deformation, normal-driven indexed-env material `3` lookup, depth-sized additive flare blits, tiled noise background, scripted flash overlays, a Java-style left-to-right horizontal smear (`RgbSurface.aMajAKK(0.875f)`), and separate Java-style temporal frame ghosting (`RgbSurface.AmajakK()`)
