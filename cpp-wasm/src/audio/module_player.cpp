...
bool lookup_sequence_module_config(const std::string& sequence_name,
    ...
    if (sequence_name == "uppol") {
        config->module_path = "original/forward/mods/jarnomix.xm";
        config->boost = 128;
        config->has_start_song_position = true;
        config->start_song_position_hex = 0x1600U;
        return true;
    }
    return false;
}
...
struct ModuleAudioStream::Impl {
    ...
    Impl() : next_tick_sample(0), cursor(0), pending_offset(0) {}
};

ModuleAudioStream::ModuleAudioStream() {}
ModuleAudioStream::~ModuleAudioStream() {}

bool ModuleAudioStream::open(const std::string& sequence_name, int sample_rate,
                             std::string* error) {
    impl_.reset();
    if (sample_rate <= 0 || (sequence_name != "intro" && sequence_name != "saari")) {
        if (error) *error = "stream requires intro or saari and a positive sample rate";
        return false;
    }
    std::unique_ptr<Impl> next(new Impl());
    if (!load_sequence_module_song(sequence_name, &next->song, &next->mad.boost,
                                    NULL, error)) return false;
    next->mad.frequency = sample_rate;
    next->song.akKaMaJ();
    impl_.swap(next);
    return true;
}

std::uint64_t ModuleAudioStream::position() const { return impl_ ? impl_->cursor : 0; }

bool ModuleAudioStream::render(std::size_t frames, SequenceAudioRender* block,
                               std::string* error) {
    if (!impl_ || !block || frames > static_cast<std::size_t>(std::numeric_limits<int>::max())) {
        if (error) *error = "invalid module stream block";
        return false;
    }
    Impl& s = *impl_;
    block->song_positions.clear();
    block->interleaved_samples.assign(frames * 2, 0);
    std::size_t written = 0;
    while (written < frames) {
        if (s.pending_offset == s.packed.size()) {
            bool emitted = false;
            unsigned int position = 0;
            s.song.process_tick(&emitted, &position);
            if (emitted) {
                SongPositionEvent event = {s.cursor, position};
                block->song_positions.push_back(event);
            }
            s.next_tick_sample += s.song.tick_duration_seconds() * s.mad.frequency;
            if (static_cast<std::uint64_t>(s.next_tick_sample) == s.cursor)
                s.next_tick_sample += s.song.tick_duration_seconds() * s.mad.frequency;
            const std::size_t tick_frames = static_cast<std::size_t>(s.next_tick_sample) - s.cursor;
            // Preserve the original mix call boundaries. Its ping-pong sample loop
            // rounds state when a mix call ends; arbitrary splits change later PCM.
            s.packed.assign(tick_frames, 0);
            s.song.mix_segment(s.mad, &s.packed, 0, static_cast<int>(tick_frames));
            s.pending_offset = 0;
        }
        const std::size_t count = std::min(frames - written, s.packed.size() - s.pending_offset);
        for (std::size_t i = 0; i < count; ++i) {
            const std::uint32_t value = s.packed[s.pending_offset + i];
            block->interleaved_samples[(written + i) * 2] = static_cast<std::int16_t>(value & 0xffffU);
            block->interleaved_samples[(written + i) * 2 + 1] = static_cast<std::int16_t>(value >> 16);
        }
        s.pending_offset += count; s.cursor += count; written += count;
    }
    return true;
}

bool module_position_sample(const std::string& sequence_name, int sample_rate,
                            unsigned int position, std::uint64_t* sample, std::string* error) {
    ModuleSong song;
    if (!load_sequence_module_song(sequence_name, &song, NULL, NULL, error)) return false;
    return find_song_position_sample_index(song, sample_rate, position, sample, error);
}
